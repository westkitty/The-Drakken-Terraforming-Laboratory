...
export class LaboratoryApp {
    ...
    this.root.innerHTML = `
      <main class="lab-shell">
        ...
        <aside class="panel rack" aria-label="Experiment rack">
          ...
          <button id="placement" class="active">PLACEMENT ARMED</button>
  ...
  private bind(): void {
    ...
    this.root.querySelectorAll<HTMLButtonElement>('[data-layer]').forEach(b=>b.addEventListener('click',()=>{this.root.querySelectorAll('[data-layer]').forEach(x=>x.classList.remove('active'));b.classList.add('active');this.currentLayer=b.dataset.layer as LayerId;this.renderer.setLayer(this.currentLayer);}));
  }

  private loop = (now:number): void => {
    const dt=Math.min(0.1,(now-this.last)/1000);this.last=now;
    if(this.playing){this.accumulator+=dt*this.speed*8;let steps=0;while(this.accumulator>=1&&steps<24){this.engine.step();this.accumulator-=1;steps++;}if(steps){this.renderer.markDirty();this.refreshLight();}}
  ...
  private refresh(): void {
    ...
    this.must('statusline').textContent=`SEED ${s.seed} · TICK ${s.tick} · BRANCH ${s.branchId} · HASH ${this.engine.hash()} · ACTIVE ${[...this.engine.processes.values()].filter(p=>p.active).length}`;
    ...
    const g=s.gorevault,o=s.orbital,ledger=this.engine.ledger();this.must('ledger').innerHTML=[['WATER MASS',m.waterMass],['CONVERTIBLE REMAINING',m.convertibleRemaining],['HARVESTED',g.totalHarvested],['PIPELINE ACCOUNTED',ledger.pipelineAccounted],['PIPELINE ERROR',ledger.pipelineError],['REFINED FEEDSTOCK',g.refinedFeedstock],['RISING MATERIAL',o.risingMaterial],['ORBITAL LOOSE',o.orbitalLooseMaterial],['SHAPED BAND',o.shapedBandMaterial],['BAND COVERAGE',o.bandCoverage],['BAND INTEGRITY',o.bandIntegrity]].map(([k,v])=>`<span>${k}<b>${fmt(Number(v))}</b></span>`).join('');
    this.must('instances').innerHTML=[...this.engine.processes.values()].map(p=>`<div><b>${display(p.processId)}</b><span>${p.id}</span><button data-toggle="${p.id}">${p.active?'DEACTIVATE':'ACTIVATE'}</button></div>`).join('')||'<p>NO ACTIVE PROCESS INSTANCES</p>';
