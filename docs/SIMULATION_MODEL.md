...
A snapshot is recorded every 25 ticks and the retained list is capped at 64 snapshots per branch. This bounds memory. Restore uses the nearest available branch snapshot at or before the target and replays deterministic actions/ticks forward from that boundary. Branches freeze inherited actions and inherited timeline events at fork time. Once a branch relationship exists, history before the fork is immutable for both the child and its direct parent; the fork tick itself remains the first editable boundary. If a new action is inserted in an allowed earlier tick, future snapshots for that branch are discarded before replay so stale state cannot survive the edit. If the original tick-zero snapshot has been evicted, replay begins from the generated baseline and still applies tick-zero actions. Derived milestone events invalidated by a past edit are regenerated during user-facing replay. Timeline reads are filtered through the currently viewed tick so rewinding cannot expose future events, and the closed-band event is generated immediately on the tick where closed state becomes true.
