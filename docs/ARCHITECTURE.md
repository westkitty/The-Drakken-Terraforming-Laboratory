...
Each branch records parent ID, fork tick, a frozen copy of inherited actions through the fork boundary, inherited timeline events through that boundary, and branch-local actions. Once a child is created, later edits to the parent cannot leak into the child's past. Shared history is immutable before an existing fork: a child cannot mutate before its own fork tick, and a parent cannot retroactively mutate before a direct child's fork tick. The exact fork tick remains editable because actions scheduled there affect the next simulated state rather than rewriting the already-shared fork state. Inserting an allowed action into an earlier editable tick truncates future snapshots for that branch so restore cannot select stale state. Derived milestone events invalidated by the edit are regenerated during user-facing deterministic replay; comparison/capture replays remain side-effect free. Timeline queries are bounded by the viewed tick, so rewind never exposes future events, and the closed-band event is emitted on the exact simulation tick that closure becomes true rather than waiting for a 25-tick milestone scan.
